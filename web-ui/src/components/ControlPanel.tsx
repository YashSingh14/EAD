...
export function ControlPanel({ chatState }: { chatState: ChatState }) {
  const { provider, setProvider, modelName, setModelName } = chatState;
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-lg p-4 shadow-sm-subtle">
      ...
      <div className="flex bg-slate-100 p-1 rounded-md mb-4">
        <button
          onClick={() => { setProvider('local'); setModelName('mistral'); }}
        ...
        <button
          onClick={() => { setProvider('cloud'); setModelName('gemini-1.5-flash'); }}
          className={`flex-1 text-xs font-medium py-1.5 rounded transition-colors ${
